...
/** Escolhe em qual conta a quitação entra (recebimento ou pagamento), antes de confirmar. */
export function SettleDebtModal({ debt, onCancel, onConfirm }: SettleDebtModalProps) {
  ...
  // Reseta pro padrão quando abre pra outra dívida (durante a renderização, sem setState no effect).
  const [openFor, setOpenFor] = useState<number | null>(null);
  if ((debt?.id ?? null) !== openFor) {
    setOpenFor(debt?.id ?? null);
    setAccount(DEFAULT_ACCOUNT_NAME);
  }
  ...
  return (
    <Modal visible={debt !== null} transparent animationType="fade" onRequestClose={onCancel}>
      <View style={{ flex: 1, backgroundColor: modalScrim(theme, 0.6), justifyContent: "center", padding: 24 }}>
        <View style={{ ...modalCard(theme), borderRadius: 20, padding: 24 }}>
          <Text style={{ color: colors.textPrimary, fontSize: 18, fontWeight: "bold", marginBottom: 8 }}>
            {isLent ? "Confirmar recebimento" : "Confirmar pagamento"}
          </Text>
          <Text style={{ color: colors.textSecondary, fontSize: 14, marginBottom: 20 }}>
            {isLent ? `Receber de ${debt.person}: ` : `Pagar a ${debt.person}: `}
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            ...
            <TouchableOpacity
              ...
              accessibilityRole="button"
            >
              <Text style={{ color: colors.textOnColor, fontSize: 15, fontWeight: "bold" }}>
                {isLent ? "Confirmar recebimento" : "Confirmar pagamento"}
              </Text>
            </TouchableOpacity>
          </View>
