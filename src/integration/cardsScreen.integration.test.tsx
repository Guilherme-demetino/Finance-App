...
describe("tela de cartões", () => {
  ...
  it("voltar usa o histórico ou cai no painel", async () => {
    const tree = await mount();

    mockRouter.canGoBack.mockReturnValue(true);
    await press(tree, "Voltar");
...
describe("importar a fatura pela tela", () => {
  ...
  const csv = () =>
    [
      ...
      `${iso(daysAgo(90))},pagamento,Pagamento recebido,-900.00`,
    ].join("\n");
  ...
  it("mostra a conferência, importa as compras e avisa em qual fatura entraram", async () => {
    ...
    expect(textOf(tree)).toContain("Ignoradas: 1 crédito/pagamento");
    ...
    expect(textOf(tree)).toMatch(/2 compras importadas na fatura de [A-Z]{3}\/\d{4}\./);
